    ...
    def format(self, fewshot: int = 0):
        def _format(
            ...
            if "cot" in example:
                cot = (
                    example["cot"]
                    if type(example["cot"]) is str
                    else "".join(example["cot"])
                )
            else:
                cot = None
            ...
            [
                {
                    ...
                    "case": _format(
                        self.examplars[i],
                        use_answer=True,
                        input_template_func=self.demo_input_template,
                    ),
                    "ctxs": self.examplars[i]["ctxs"]
                    if "ctxs" in self.examplars[i]
                    else [],
                }
        ...
        def _format_for_dataset(example):
            # case
            case = _format(
                example, use_answer=False, input_template_func=self.test_input_template
            )
            # ctx
            example["demo"] = demo
            example["case"] = case

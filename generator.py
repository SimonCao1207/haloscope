...
class BasicGenerator:
    ...
    def generate(
        ...
        if return_logprobs:
            ...
            if num_return_sequences == 1:
                ...
                tokens = [
                    [self.tokenizer.decode(t) for t in each_generated_tokens]
                    for each_generated_tokens in generated_tokens
                ]
                logprobs = [
                    [p.cpu().numpy().item() for p in logprobs]
                    for logprobs in transition_scores
                ]
                assert len(tokens) == len(logprobs)
            else:
                tokens = [
                    [self.tokenizer.decode(t) for t in generated_tokens[i]]
            ...
            if return_entropies:
                ...
                entropies = -np.sum(
                    softmax_probs * np.log(softmax_probs + 1e-10), axis=-1
                )
    ...
    def generate_attn(
        self, input_text, max_length, solver="max", use_entropy=False, use_logprob=False
    ):
        ...
        for i in range(len(input_text)):
            ...
            for j, t in enumerate(tokens):
                if (
                    ...
                    or generated_tokens[i][j] in self.sep_ids
                    or (j > 0 and tokens[j - 1] == "</s>")
                ):
                    range_.append([j, j])
                else:
                    range_[-1][-1] += 1
            ...
            # attention
            atten = self.model(
                generated_tokens[i].unsqueeze(0), output_attentions=True
            ).attentions[-1][0]
            if solver == "max":
